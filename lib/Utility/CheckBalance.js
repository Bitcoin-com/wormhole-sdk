...
var CheckBalance = function () {
  ...
  _createClass(CheckBalance, [{
    key: "checkBalance",
    value: async function checkBalance(cashAddress) {
      var finalBalance = {};
      var balance = await this.Wormhole.Address.details([cashAddress]);
      finalBalance.bch = balance[0].balance;
      finalBalance.satoshis = balance[0].balanceSat;

      // get token balances
      try {
        var tokens = await this.Wormhole.DataRetrieval.balancesForAddress(cashAddress);
